...
class TestRequestsTestCase(BaseTestCase):
    ...
    def test_user_make_new_request(self):
        ...
        #correct request
        response = self.app.post('/api/v2/requests/', data=json.dumps(
        ...
        data = json.loads(response.get_data())
        self.assertEqual(data['message'],'Please enter category as either repair or maintenance')
        #no title
        response = self.app.post('/api/v2/requests/', data=json.dumps(
        ...
        self.assertEqual(response.status_code, 400)
        data = json.loads(response.get_data())
        self.assertEqual(data['message'],'Frequency must be a string.')


    def test_user_view_all_requests(self):
        """Test for viewing all requests"""

        response = self.app.get('/api/v2/admin/requests/')
    ...
    def test_user_view_a_users_requests(self):
        ...
        response = self.app.get('/api/v2/requests/')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.get_data())
        self.assertEqual( data['message'],"all requests found successfully")
    ...
    def test_user_view_a_request(self):
        ...
        response_message = self.app.get('/api/v2/requests/6')
    ...
    def test_user_modify_a_request(self):
        ...
        # response = self.app.put('/api/v2/requests/5',
        #                                 data=json.dumps(
    ...
    #     response = self.app.delete('/api/v2/requests/5')
    #     self.assertEqual(response.status_code, 200)
    #     data = json.loads(response.get_data())
